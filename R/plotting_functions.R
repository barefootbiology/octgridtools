...
ggplot_pca2 <- function(x, pc1=1, pc2=2, info) {
    ...
    x$ind$coord %>%
        ...
        inner_join(info)%>%
        distinct() %>%
        ggplot(aes_string(x = paste("Dim.", pc1, sep=""),
                          y = paste("Dim.", pc2, sep=""))) +
...
ggplot_coc <- function(x, pc1, pc2, labels=c(pc1, pc2)) {
    ...
    correlations_mean <- correlations %>%
        group_by(seg_layer) %>%
    ...
    ggplot() +
        ...
        geom_segment(data = correlations,
                     aes(x = x1, y = y1,
                         ...
                         color = seg_layer),
                     alpha = 0.4) +
        geom_text(data = correlations_mean,
                  aes(x = x2, y = y2,
                      label = seg_layer,
                      color = seg_layer),
                  fontface="bold") +
...
ggplot_pca3_mod <- function(x, y, pc1=1, pc2=2, info) {
    ...
    x %>%
        ...
        inner_join(info)%>%
        distinct() %>%
        ggplot(aes_string(x = paste("PC", pc1, sep=""),
                          y = paste("PC", pc2, sep=""))) +
        geom_hline(yintercept = 0, color = "gray65", alpha = 0.5, size = 0.5) +
        geom_vline(xintercept = 0, color = "gray65", alpha = 0.5, size = 0.5) +
        labs(x = labels[1], y = labels[2])  +

...
data(sdoct, package = "sdoctcontrol")
...
data(layer_data, package = "heyexr")
...
9. Constructing multi-panel displays for 
10. Advanced: Registering en face images from SS-OCT with color fundus images.
...
component and intensity, etc. [Alternatively, show how you can combine multiple
raster images as layers to construct an new en face image, bypassing ggplot2's
ban against mapping multiple arguments to `fill`.]
...
p_slo +
  ...
  geom_text(data = example$volume$bscan_headers %>%
               filter(bscan_id %in% seq(from = 1, to = 61, by = 3)),
            mapping = aes(label = bscan_id,
                          x = start_x_pixels,
                          y = start_y_pixels),
            color = "white", 
            size = 2.5,
...
```{r }
construct_bscan(example$volume, bscan_id = test_bscan_id) +
  geom_line(data = get_segmentation(example$volume) %>% filter(bscan_id == 30),
            mapping = aes(x = x, 
                          y = z, 
                          group = surface_id, 
...
segmentation_ira <- example$segmentation$layers %>% 
  add_undefined(get_undefined(example$segmentation)) %>%
  inner_join(invisible(layer_info))
...
We can create basic surface maps of the segmentation several ways. For example,
here is the surface map of the ILM.
...
```{r }
enface_retinal_thickness_difference <- 
  enface_retinal_thickness %>% 
...
p_difference_map + 
  p_scatterplot + 
  plot_layout(nrow = 1, widths = c(4, 3))
```
...
slicer_full_thickness <- create_slicer_range(seg_volume = seg_volume, 1, 10)

slicer_full_thickness[ , 30, ] %>%
  melt_array(c("x", "z", "intensity")) %>%
  ggplot(aes(x = x, y = z)) +
  geom_raster(aes(fill = intensity)) +
  scale_y_reverse()
```
...
layers_flattened <- example$segmentation$layers %>% 
  ...
  mutate(value_flattened = max(value_defined) - value_defined) %>%
  ungroup() 

layers_flattened %>% 
  filter(bscan_id == 30) %>% 
  ggplot(aes(x = ascan_id, y = value_flattened)) + 
  geom_line(aes(group = surface_id, 
                color = as.factor(surface_id))) +
  scale_color_brewer(palette = "Paired")
```
...
layers_flattened %>% 
  ...
  geom_line(aes(group = surface_id, 
                color = as.factor(surface_id))) +  
  geom_ribbon(aes(group = surface_id, 
                  ymax = value_flattened,
                  ymin = 0,
                fill = as.factor(surface_id))) +
  scale_fill_brewer(palette = "Paired") +
  scale_color_brewer(palette = "Paired")
```
...
# Use an affine matrix to flip the ETDRS grid to work in the SLO space
affine_matrix <- matrix(c(1, 0, 0,
                         0, -1, 0,
                         0, 0, 1), 
                        ncol = 3, 
                        byrow = TRUE)
...
grid_mm <- 
  etdrs_grid %>% 
  affine_transform_coord(affine = affine_matrix) %>%
...
surfaces_enface <- 
  ...
  map(~.x * get_undefined_matrix(example$segmentation)) %>%
...
surface_enface_tbl <- 
  surfaces_enface %>%
  map_dfr(~melt_array(.x, c("x", "y", "value")), 
          .id = "layer") %>%
  as_tibble() %>%
...
# Projecting data in three dimensions

We can plot each segmentation surface in 3D. To do this correctly, we'll have
to create a few object to flesh out the visualization.
...
surface_colors <- RColorBrewer::brewer.pal(length(surface_array_list), 
                                           "Paired")
...
size_x <- example$segmentation$info$size_x
size_z <- example$segmentation$info$size_z
...
surface_enface_colors <- 
  ...
  map(~octgridtools::na_replace(x = .x, 257))  %>%
  map(~gray_scale[.x])
...
Can we pass `NA` as the color instead?

```{r }
surface_enface_colors_NA <- 
  surface_enface_colors %>%
  map(~if_else(.x != "green", .x, as.character(NA)))
...
surface_array_list %>%
  ...
  iwalk(~surface3d(x = 1:size_x,
                 y = 1:size_z,
                 z = flip_array(.x, 2), 
                 color = surface_enface_colors_NA[[.y]],
                 add = TRUE))
...
```{r }
leading_edge <- seq(from = example$grid_center$center$z, 
                    to = 1, 
                    length.out = length(surface_array_list)) %>% 
  round() %>%
  set_names(as.character(1:length(surface_array_list)))

surface_enface_colors_trimmed <- 
  surfaces_enface %>%
  imap(~255 * flip_array(.x, 2)[, leading_edge[[.y]]:size_z ] + 1) %>%
  map(~octgridtools::na_replace(x = .x, 257))  %>%
  map(~gray_scale[.x])
...
surface_array_list %>%
  (function(x) set_names(x, as.character(1:length(x)))) %>%
  iwalk(~surface3d(x = 1:size_x,
                 y = leading_edge[[.y]]:size_z,
                 z = flip_array(.x, 2)[, leading_edge[[.y]]:size_z ], 
                 color = surface_enface_colors_trimmed[[.y]],
                 add = TRUE))

rglwidget()
```

The problem with this plot is that it's difficult to distinguish the layers,
even after trimming them. What we can do is combine unique colors for the layers
as well as the structural information. 

But right now I'll just show single colors for each layer:
...
Alternatively, we can just cut off all the values at the fovea:
...
surface_array_list %>%
  (function(x) set_names(x, as.character(1:length(x)))) %>%
  iwalk(~surface3d(x = 1:size_x,
                 y = example$grid_center$center$z:size_z,
                 z = flip_array(.x, 2)[ , example$grid_center$center$z:size_z ], 
                 color = surface_colors[[as.numeric(.y)]],
                 add = TRUE))
